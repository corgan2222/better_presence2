...
def test_stale_gps_home_not_returned_as_home_via_zone_loop():
    ...
    coord = make_coordinator(hass)
    # Must return not_home, not home — stale GPS should not pass through any loop
    assert coord._get_aggregate_state(["device_tracker.gps"]) == "not_home"


def test_initial_home_sets_home_without_timer():
    """On first evaluation from empty state with home tracker, go directly to Home."""
    hass = make_hass({"device_tracker.thomas_ping": {"state": "home"}})
    coord = make_coordinator(hass)
    with patch.object(coord, "_start_timer") as mock_timer:
        coord._evaluate_person("thomas")
